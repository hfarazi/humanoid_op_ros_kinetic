...
void Diagnostics::initPlugin(qt_gui_cpp::PluginContext& context)
{
	...
	if(ros::Time::isValid())
	{
		m_torqueClient.reset(new actionlib::SimpleActionClient<robotcontrol::FadeTorqueAction>(getNodeHandle(), "/robotcontrol/fade_torque"));
		m_fadeAllowed = true;
	}
...
void Diagnostics::update()
{
	...
	m_ui.tempLabel->setText(QString("%1C").arg(m_diag->servoTemperature, 0, 'f', 1));
